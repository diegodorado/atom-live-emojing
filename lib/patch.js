  ...
  return text.replace(/(s)(?:ound)?\s+"(.*?)"/g, (matchedString, sound, string) => {
    ...
    for (c of string) {
      if (c.codePointAt(0) < 128) {
        ...
        console.log(c, map[c], map[""], !map[c], !map[""])
        if (map[c]){
          ...
          // is there a fallback mapping defined?
          if(map[""]){
            sounds.push("["+map[""]+"]")
            emojis.push("["+c.codePointAt(0)+"]")
          }else{
...
export {patchTidalCycles, loadMap}
